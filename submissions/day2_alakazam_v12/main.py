...
def _raw_legal_fallback(obs_dict):
    """Return a legal raw-observation fallback when parsing unexpectedly fails."""
    ...
    if not isinstance(options, list) or not options:
        return []
    try:
        minimum = max(0, int(select.get("minCount", 1)))
        maximum = int(select.get("maxCount", minimum))
    except (TypeError, ValueError):
        minimum, maximum = 1, 1
    if maximum <= 0:
        return []
    count = min(max(maximum, 0), len(options))
    count = max(minimum, count)
    if count > len(options):
        return []
    return list(range(count))

# Generated submission hardening: exact deck, independent of process cwd.
_EMBEDDED_DECK = [5, 5, 13, 19, 19, 19, 19, 66, 66, 140, 305, 305, 305, 343, 741, 741, 741, 741, 742, 742, 742, 742, 743, 743, 743, 743, 1079, 1079, 1079, 1081, 1081, 1081, 1081, 1086, 1086, 1086, 1086, 1097, 1129, 1152, 1152, 1152, 1152, 1182, 1182, 1182, 1184, 1197, 1197, 1197, 1225, 1225, 1225, 1225, 1231, 1231, 1231, 1231, 1266, 1266]
...
# V12 public-field refinement: Team Rocket's Energy provides two Energy while
# attached. Its card ID and attached location are visible in the official
# observation, so this rule does not infer a card from a hidden zone.
...
def _rocket_energy_hammer_scores(obs):
    state = obs.current
    select = obs.select
    # The parent search may query a transient simulation observation without a
    # selectable action.  There is no score to adjust in that state; calling
    # the parent heuristic would dereference select.context and crash.
...
# The competition's initial deck-query observation may omit the optional step
# field for one side.  Answer it before the parent parser runs: the parent
# fallback otherwise returns a one-element legal-action list, which the engine
# correctly rejects as a deck.
_parent_agent = agent
